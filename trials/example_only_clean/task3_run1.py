class LoanwordApiError(Exception):
    """외래어 표기 API 호출 자체가 실패했음을 나타낸다 (인증/서버 오류 등)."""
...
def get_standard_notation(word: str) -> str | None:
    ...
    if response.status_code == 403:
        raise LoanwordApiError(f"요청 거부됨(403, 헤더 누락 의심): word={word!r}")
    if response.status_code != 200:
        raise LoanwordApiError(f"API 호출 실패: status_code={response.status_code}")
