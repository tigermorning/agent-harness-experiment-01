...
class DialectAPIError(RuntimeError):
    ...
    def __init__(self, word: str, status_code: int) -> None:
        super().__init__(
...
def find_dialect_matches(word: str) -> list[str]:
    ...
    정상 응답이면 바디의 `matches` 리스트를 그대로 돌려준다.
    200이 아닌 응답은 조회 실패이므로 `DialectAPIError`를 발생시킨다.
    "매칭이 없음"(빈 리스트)과 "조회하지 못함"은 서로 다른 결과이므로
    실패를 빈 리스트로 바꿔 삼키지 않는다.
    """
